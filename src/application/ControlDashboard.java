...
public class ControlDashboard {
	...
	@FXML
	void crop4goc(ActionEvent event) {
		ImageProcessing process = new ImageProcessing(this.workingImage);
		File file = new File(this.images.get(tumbnails[0]));
		writeConsole(">> Crop4goc <"+ file.getName() +">:");
		writeConsole(process.Crop4goc());
		this.displayImage(workingImage);
	...
	public void data(File file){
		...
		try {
			...
			e.printStackTrace();
			this.writeConsole("Error with metadata");
			return;
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			this.writeConsole("Error2 with metadata");
			return;
		}
